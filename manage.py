...
@app.route('/dnsscannerinput',methods=["POST"])
def dnsscannerinput():
    if request.method == "POST":
        ipAddress = request.form["host"]
        print(ipAddress)
        from dnsrecord import dnsScan
        dictionary=dnsScan(ipAddress)
        total=next(iter(dictionary))
        addrDict = dictionary[total]
        for ipAdd in addrDict:
            print(ipAdd)
        return render_template('dns.html',ipAddress=ipAddress,addrDict=addrDict,total=total)
...
@app.route('/ipscannerInput',methods=["POST"])
def ipscannerinput():
    if request.method == "POST":
        ipAddress = request.form["host"]
        print(ipAddress)
        from ipscanner import ipAddresScan
        dictionary=ipAddresScan(ipAddress)
        total=next(iter(dictionary))
        addrDict = dictionary[total]
        return render_template('ipscanner.html',ipAddress=ipAddress,addrDict=addrDict,total=total)
    else:
        return EnvironmentError
